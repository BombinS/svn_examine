...
def commandExportFile(revision, svnPath, filePath):
    command = 'svn export -r {} "{}{}" "./workspace"'.format(revision, svnPath, filePath)
    return command

def commandIsFileExistAtLastRevison(filename, extension, path, svnRoot):
    command = 'svn ls {}{}/{}.{}'.format(svnRoot, path, filename, extension)
    return command
